...
export default function OnboardingPage() {
  ...
  const saveProfile = async (event: FormEvent) => {
    ...
    try { setProfile(await api.updateProfile(profile)); setSuccess("Profile saved — one step closer."); await load(); }
    catch (reason) { setError(apiErrorMessage(reason)); }
  ...
  const saveAssessment = async (event: FormEvent) => {
    ...
    if (!allAnswered) { setError("Almost there — answer all five questions before submitting."); return; }
    setBusy(true); setError(""); setSuccess("");
    try { setAssessment(await api.updateSuitability({ ...assessment, knowledge_score: knowledgeScore })); setSuccess(knowledgeScore >= 3 ? "Nice work — this step is done." : "Saved. Have a look at the Learn section, then retake the knowledge check to unlock real-money trading."); await load(); }
    catch (reason) { setError(apiErrorMessage(reason)); }
    finally { setBusy(false); }
  };
  ...
  const acknowledge = async () => {
    ...
    setBusy(true); setError(""); setSuccess("");
    try { await api.acknowledgeRisk(status.disclosure_version); setSuccess("Thanks — your risk acknowledgement is recorded."); await load(); }
    catch (reason) { setError(apiErrorMessage(reason)); }
    finally { setBusy(false); }
  };
  ...
  const steps = [
    ["Your details", status?.profile_complete, UserRoundCheck],
    ["Your experience", status?.suitability_complete, FileCheck2],
    ["Know the risks", status?.risk_acknowledged, ShieldAlert],
    ["Connect Deriv", status?.deriv_connected, Link2],
  ] as const;

  return (
    <>
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-[11px] font-bold uppercase tracking-[.17em] text-black/30">Get set up</p><h1 className="mt-2 text-[36px] font-medium leading-none tracking-[-.05em] sm:text-[44px]">Let's get you trading</h1><p className="mt-3 max-w-[670px] text-sm font-medium leading-relaxed text-black/40 sm:text-base">You can practise with demo funds as soon as you connect Deriv. To trade with real money, we just need a few details first — it's there to protect you.</p></div>{status?.ready_for_live && <span className="inline-flex items-center gap-2 rounded-full bg-green-100 px-4 py-2.5 text-xs font-bold text-green-800"><BadgeCheck size={15}/> Ready for live trading</span>}</div>
