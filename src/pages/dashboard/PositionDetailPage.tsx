...
export default function PositionDetailPage() {
  ...
  const update = async (event: FormEvent) => {
    ...
    try {
      ...
      setSuccess("Protection saved. Your trade will now close automatically at your levels."); setStopLoss(""); setTakeProfit(""); await load();
    } catch (reason) { setError(apiErrorMessage(reason)); }
    finally { setBusy(false); }
  };

  const sell = async () => {
    if (!window.confirm("Close this trade at the current market price?")) return;
  ...
  const cancel = async () => {
    if (!window.confirm("Cancel this trade? Only some trades allow cancellation.")) return;
    setBusy(true); setError("");
    try { await api.cancelContract(activeLoginID, parsedID); navigate("/app/portfolio"); }
    catch (reason) { setError(apiErrorMessage(reason)); setBusy(false); }
  };

  if (!activeAccount) return <div className="rounded-[22px] border border-black/[.07] bg-[#f7f7f4] p-10 text-center"><p className="text-sm text-black/40">Connect your Deriv account to see this trade's details.</p></div>;
  ...
  return (
    <>
      ...
      <div className="mt-6 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-[11px] font-bold uppercase tracking-[.17em] text-black/30">Live trade</p><h1 className="mt-2 text-[36px] font-medium leading-none tracking-[-.05em] sm:text-[44px]">Trade #{parsedID}</h1><p className="mt-3 text-sm font-medium text-black/40">{textValue("longcode")}</p></div><button type="button" onClick={() => void load()} className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/50 px-5 py-3 text-sm font-semibold"><RefreshCw size={15}/> Refresh</button></div>
